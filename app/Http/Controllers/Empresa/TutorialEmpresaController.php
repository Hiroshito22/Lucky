...
use App\Models\TutorialEmpresa;
use App\User;
use Exception;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class TutorialEmpresaController extends Controller
{
    /**
     * Crea un Empresa Soporte
     * @OA\POST (
     *     path="/api/empresa/soporte/create",
     *     summary="Crea un soporte con sesión iniciada",
     *     security={{ "bearerAuth": {} }},
     *     tags={"Empresa Soporte"},
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            TutorialEmpresa::create([
                "nombre" => $request->nombre,
                "bregma_id" => $userbregma->user_rol[0]->rol->bregma_id,
                "link" => $request->link
            ]);
            DB::commit();
            return response()->json(["resp" => "Empresa Soporte creado"]);
        } catch (Exception $e) {
            DB::rollback();
    ...
    /**
     * Actualiza un Soporte Empresa
     * @OA\PUT (
     *     path="/api/empresa/soporte/update/{id}",
    ...
    public function update(Request $request, $Idsoporte)
    {
        DB::beginTransaction();
        try {
            $soporte = TutorialEmpresa::find($Idsoporte);
            if ($soporte) {
                $soporte->fill(array(
                    "nombre" => $request->nombre,
                    "link" => $request->link
     ...
     * Elimina un Soporte
     * @OA\DELETE (
     *     path="/api/empresa/soporte/delete/{id}",
    ...
    public function delete($Id)
    {
        try {
            DB::beginTransaction();
            $soporte = TutorialEmpresa::find($Id);
            if ($soporte) {
                $soporte->fill([
                    "estado_registro" => "I",
                ])->save();
    ...
    public function show()
    {
        try {
            $userbregma = User::with('user_rol.rol')->where('id', auth()->user()->id)->first();
            $bregmalocal = TutorialEmpresa::with('bregma.tipo_documento')
